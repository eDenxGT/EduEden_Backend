		...
		if (role === "tutor") {
			if (user.is_identity_verified === "pending") {
				return res.status(403).json({
					message:
						"Account verification in progress. Please try again later.",
...
const sendOtpForProfileUpdate = async (req, res) => {
	try {
		...
		if (!role || !email) {
			return res
				.status(400)
				.json({ error: "Role and email are required." });
		}
...
const verifyOtpProfileUpdate = async (req, res) => {
	try {
		...
		if (otpEntry.used) {
			return res
				.status(400)
				.json({ error: "OTP has already been used." });
		}
		...
		return res.status(200).json({ message: "Email verified. Click on Save Changes to update." });
	} catch (err) {
