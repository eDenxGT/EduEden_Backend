...
const generateAccessToken = (role, data) => {
	if (role === "student") {
		return jwt.sign({ _id: data?._id, email: data?.email, role: data?.role }, process.env.JWT_STUDENT_ACCESS_TOKEN_SECRET, {
		...
		return jwt.sign({ _id: data?._id, email: data?.email, role: data?.role }, process.env.JWT_TUTOR_ACCESS_TOKEN_SECRET, {
			expiresIn: process.env.JWT_ACCESS_TOKEN_EXPIRES,
		});
	}
};
